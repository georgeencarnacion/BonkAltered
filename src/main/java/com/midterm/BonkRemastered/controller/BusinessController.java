...
public class BusinessController {
    ...
    @PostMapping("/{userId}")
    private String adduserBusiness(@PathVariable Long userId, BusinessDTO businessDTO,UserDTO userDTO,Authentication auth, Model model){
        businessDTO.setOwner(userId);
        businessService.add(businessDTO);
        smsSender.sendSms(userDTO, auth);
